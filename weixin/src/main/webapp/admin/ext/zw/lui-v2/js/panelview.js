	...
	me.config = {
		...
		callback: {
			loadCallback: function ($addNode, panelO) {
				...
				//绑定移除事件
				panelO.$content.delegate("[data-role='remove']", "click._removeNode", function (event) {
					//停止冒泡至节点点击事件
				...
				if (me.config.view.enabledcontrol) {
					//解除委派事件
					me.$control.undelegate("[data-role='yes']", "click._yesCallback");
					//存在控制区时，绑定默认事件
					//默认控制器确认事件
					me.$control.delegate("[data-role='yes']", "click._yesCallback", function (event) {
						//自定义点击事件
						if (me.config.callback.yesCallback && $.type(me.config.callback.yesCallback) == "function") {
							me.config.callback.yesCallback($(this), me);
						}
					});
					me.$control.undelegate("[data-role='no']", "click._noCallback");

					//默认控制器取消事件
					me.$control.delegate("[data-role='no']", "click._noCallback", function (event) {
						//自定义点击事件
			...
			nodeClick: function ($currentNode, currentData, panelO) {
				me.config.action.selectNode($currentNode, currentData, panelO);
			},
...
Panelview.prototype = new LUIControllerAdvance();
