...
(function (factory) {
	...
	var settings = {
		//下拉html
		selectHtml: '<div class="ui-selectbox select-multiply">' +
		...
		//标签html
		labelHtml: '<li class="ui-selectbox-label-item" data-value="{{value}}">' +
			'<span>{{name}}</span>' +
			'<i class="iconfont remove-option">&#xe628;</i>' +
			'</li>',
		//选项html
		optionHtml: '<li class="ui-selectbox-drop-option" data-value="{{value}}" data-selected="{{isSelected}}">' +
		...
		init: function (options) {
			...
			return this.each(function () {
				...
				if (!data) {
					...
					//注册事件
					addListeners($simulator, $this, settings);
	...
	//侦听器
	function addListeners($simulator, $dom, settings) {
		$(document).bind('click', function (event) {
			$simulator.find('.ui-selectbox-drop').hide();
			$simulator.data('isOpen', false);
			
			if ($.type(settings.callback.closeCallback) == "function") {
				settings.callback.closeCallback($simulator, $dom, settings);

			}
		});

		$simulator.find('.ui-selectbox-label').bind('click', function (event) {
			if (!settings.enableDroplist) {
				return false;
			}
