...
/**
 * Console控制台打印信息
 * @param {All} msg   打印信息，样式通过%c控制
 * @param {String} style 打印信息的样式
 */

function log(msg, style) {
	//控制台打印模式未开启或不支持console对象时，退出
	if (!logDebug || !console) {
		return;
	}
...
//
/**
 * 在页面上打印信息：会打印出数据的值和数据类型，数组还会打印序号，原生对象还会打印键名
 * @param {All} msg   任何的数据
 * @param {String||number} namespace 打印目标选择符，未指定输出区域时默认在页面底部输出
 */
function output(msg, namespace) {
	...
	//封装输出HTML格式
	var $content = $("<div class='output-content'></div>");

	if ($.type(msg) == "array") {
		var $table = $("<table/>");
		$table.append("<caption>值：" + msg + "，类型:" + $.type(msg) + "</caption>");
		$table.append("<tr><th>序号</th><th>值</th><th>类型</th></tr>");
		for (var i = 0; i < msg.length; i++) {
			var $tr = $("<tr/>");
			$tr.append("<td>" + i + "</td>").append("<td>" + msg[i] + "</td>").append("<td>" + $.type(msg[i]) + "</td>");
			$table.append($tr);
		...
		var $table = $("<table/>");
		$table.append("<caption>值：" + msg + "，类型:" + $.type(msg) + "</caption>");
		$table.append("<tr><th>键</th><th>值</th><th>类型</th></tr>");
		for (var key in msg) {
			var $tr = $("<tr/>");
			$tr.append("<td>" + key + "</td>").append("<td>" + msg[key] + "</td>").append("<td>" + $.type(msg[key]) + "</td>");
			$table.append($tr);
			$content.append($table);
		}
	} else {
		var $table = $("<table/>");
		$table.append("<tr><th>值</th><th>类型</th></tr>");
		var $tr = $("<tr/>");
		$tr.append("<td>" + msg + "</td>").append("<td>" + $.type(msg) + "</td>");
		$table.append($tr);
		$content.append($table);
	}
	...
	//确定打印输出区域
	var $toolOutput;
	if (namespace && $.type(namespace) == "string" && $(namespace).length > 0) {
		//若是选择符，则查找这个节点
		$toolOutput = $(namespace);
	} else {
		//未指定为选择符时，在body下查找或创建
		//查找容器
		$toolOutput = $("body").find("[data-role='tool-output']");
		if ($toolOutput.length <= 0) {
			//不存在，则创建对应的调试器
			$toolOutput = $("<div class='tool-output active' data-role='tool-output'></div>");
			$toolOutput.append("<div class='tool-btn tool-btn-expand'>收起</div>");
			$toolOutput.append("<div class='tool-btn tool-btn-close'>关闭</div>");
			$("body").append($toolOutput);
			//绑定事件
			$toolOutput.on("click", ".tool-btn-expand", function (event) {
				event.stopPropagation();
				//判断当前调试器是否处于活动状态
				if ($toolOutput.is(".active")) {
					//处于活动中
					$toolOutput.removeClass("active");
					$(this).text("展开");
				} else {
					//处于活动中
					$toolOutput.addClass("active");
					$(this).text("收起");
				}
			});

			$toolOutput.find(".tool-btn-close").bind("click", function (event) {
				event.stopPropagation();
				$toolOutput.remove();
			});

		} else {
			$toolOutput.find(".output-content").remove();
		}
	}
	$toolOutput.append($content);
}
...
function _dataToArray(data) {
	...
	//只有一条数据时直接返回第一条数据，多条数据时返回的可能是二维数组
	if ($.type(data) != "array") {
...
/**
 * 将数组格式转换成单一个对象数据格式，假如数组格式中存在多个元素，则过滤掉其他元素，只返回第一个数组转换后的对象格式。对象数据类型不作转换，其他数据类型不支持转换
 * @param   {PlainObject||Array} data     数据源可以是直接健/值对的原生对象格式，也可以是多项健/值对的数组对象
 * @returns {String}   返回字符串或者字符串数组
 */
function _dataToObject(data) {
	var filterArgs = _filterArguments(arguments, [["plainobject", "array"]]);

	//输出错误信息，快速定位错误
	if (filterArgs === false) {
		var errorText = "%ccommon._dataToObject(data)";
		log(errorText, "color:#f00");
		return false;
	}
...
function addEventListener(selector, type, listener, useCapture) {
	selector.addEventListener ? selector.addEventListener(type, listener, useCapture) : "" ;
}

//移除事件
function removeEventListener(selector, type, listener, useCapture) {
	selector.removeEventListener ? selector.removeEventListener(type, listener, useCapture) : ""  ;
}*/