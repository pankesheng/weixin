	...
	$.fn.searchDroplist = function (controllerO, filterKey, matchMode) {
		...
		if (!$search.val() && dlO.getSelectedNode().length > 0) {
			if (dlO.config.view.enabledMultiple) {
				//多选
				$search.val(_dataToArray(dlO.getSelectedData("name")).join(","))
			} else {
				$search.val(_dataToArray(dlO.getSelectedData("name"))[0])
			}
		}
		...
		if (dlO.config.view.enabled) {
			...
			$search.bind("keyup", function () {
				...
				if (searchVal.length > 0) {
					var resultNodeId = dlO.filterNode($allNode, searchVal);
		...
		dlO.$content.delegate("[data-role='node']", "click", function (event) {
			...
			if (dlO.config.view.enabledMultiple) {
				...
				//如果已选中状态，则是取消，text里删除
				//拼装到原来已有的内容里
				//取得当前值
				//选中时

				var curData = dlO.getDataById($(this));
				if (curData.state == "selected") {
					//选中时
					if ($.trim($search.val()) == "") {
						$search.val(curData.name);
					} else {
						$search.val($search.val() + "," + curData.name);
					}
				} else {
					//取消选中时
					var searchText = $search.val();
					var $selectedNode = dlO.getSelectedNode();
					if ($selectedNode.length > 0) {
						var regexp1 = "/" + curData.name + ",/";
						var regexp2 = "/," + curData.name + "/";
				...
				/*var searchText = "";
				if (dlO.getSelectedNode().length > 0) {
					searchText = _dataToArray(dlO.getDataById(dlO.getSelectedNode(), "name")).toString();
					$search.val(searchText);
				} else {
					$search.val(searchText);
				}*/
...
function SelectBoxPanelview(controllerO) {
	...
	//重载面板配置
	if (!panelO.config._isUserDefined) {
		var panelOsetting = {
			//功能配置
			view: {
				//默认启用控制栏
	...
	if ($sb.is(":hidden")) {
		$panel.bind("click", function () {
			...
			var popupIndex = popupLayer(content, title, size, btn, function (index, layero) {
					...
					if (panelO2) {
						if (sbO.config.view.enabledMultiple) {
							var $panelO2Node = panelO2.getNodeById(_dataToArray(sbO.getDataById($oldSelectedNode, "id")));
